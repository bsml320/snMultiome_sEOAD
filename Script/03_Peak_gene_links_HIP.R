...
write.csv(links_clean, file = "./Results/LINK/linkpeaks_all_HIP_1.23.csv",row.names = F)
...
p1 <- df %>%
  ggplot( aes(x=Freq)) +
    geom_histogram(fill="#3f78c199", alpha=0.7, position = 'dodge',binwidth = 2,colour='#3f78c199',size=1) +
    scale_fill_viridis(discrete=TRUE)+
    xlab("Linked peaks per gene")+
    ylab("Number of genes")+
    labs(fill="HIP")+theme_classic()+ggtitle(paste("HIP: ",avg.peaks,"linked peaks per DEG."))

pdf(file = "./Figures/LINK/Hist_link.peaks_HIP_1.23.pdf",width = 5,height = 5)
p1
dev.off()
...
linked_peaks <- read.csv("./Results/LINK/linkpeaks_all_HIP_1.23.csv",header = 1)
...
write.csv(linked_peaks_annotated, file = "./Results/LINK/HIP_linkpeaks_all_annotated_1.23.csv", row.names = F)